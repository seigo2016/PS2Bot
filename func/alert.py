...
class Alert(commands.Cog):
    ...
    async def notice_alert(self):
        params = {"game_code": "ps2", "name": "SolTech (Asia)"}
        ...
        pop_time = datetime.fromtimestamp(pop_json_data["timestamp"], tz=UTC)
        ...
        ax.legend(
            ncol=len(category_names), bbox_to_anchor=(1.2, 1), loc="upper right", borderaxespad=0, fontsize="small"
        )
        ...
        with io.BytesIO() as sio:
            ...
            channel = self.bot.get_guild(self.server_id).get_channel(self.alert_channel_id)
            await channel.purge(limit=1)
            await channel.send(embed=em, file=discord.File(sio, filename="image.png"))
